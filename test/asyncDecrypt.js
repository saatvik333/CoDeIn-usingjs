...
const aclAdd = parsedEnvACL.ACL_CONTRACT_ADDRESS.replace(/^0x/, '')
    .replace(/^0+/, '')
    .toLowerCase();
...
const parsedEnv = dotenv.parse(
    fs.readFileSync('node_modules/fhevm/gateway/.env.gateway'),
);
const privKeyRelayer = process.env.PRIVATE_KEY_GATEWAY_RELAYER;
const relayer = new ethers.Wallet(privKeyRelayer, ethers.getDefaultProvider());
...
const ifaceEventDecryption = new ethers.utils.Interface([
    'event EventDecryption' + argEvents,
]);

const argEvents2 = '(uint256 indexed requestID, bool success, bytes result)';
const ifaceResultCallback = new ethers.utils.Interface([
...
export const asyncDecrypt = async () => {
    firstBlockListening = await ethers.provider.getBlockNumber();
    if (
        networkName === 'hardhat' &&
        process.env.SOLIDITY_COVERAGE_RUNNING !== 'true'
    ) {
        // evm_snapshot is not supported in coverage mode
        await ethers.provider.send('set_lastBlockSnapshotForDecrypt', [
            firstBlockListening,
        ]);
    }
    ...
    await gateway.on(
        ...
        async (
            ...
            console.log(
                `${await currentTime()} - Requested decrypt on block ${blockNumber} (requestID ${requestID})`,
            );
        },
    );
    await gateway.on(
        'ResultCallback',
        async (requestID, success, result, eventData) => {
            const blockNumber = eventData.log.blockNumber;
            console.log(
                `${await currentTime()} - Fulfilled decrypt on block ${blockNumber} (requestID ${requestID})`,
            );
        },
    );
};

export const awaitAllDecryptionResults = async () => {
    gateway = await ethers.getContractAt(
        'fhevm/gateway/GatewayContract.sol:GatewayContract',
        parsedEnv.GATEWAY_CONTRACT_PREDEPLOY_ADDRESS,
    );
    ...
    if (
        ...
        // evm_snapshot is not supported in coverage mode
        lastBlockSnapshotForDecrypt = await provider.send(
            'get_lastBlockSnapshotForDecrypt',
        );
    ...
    if (
        ...
        // evm_snapshot is not supported in coverage mode
        await provider.send('set_lastBlockSnapshotForDecrypt', [
            firstBlockListening,
        ]);
    }
};
...
const getAlreadyFulfilledDecryptions = async () => {
    let results = [];
    const eventDecryptionResult = await gateway.filters
        .ResultCallback()
        .getTopicFilter();
    ...
    results = results.concat(
        pastResults.map(
            (result) => ifaceResultCallback.parseLog(result).args[0],
        ),
    );
...
const fulfillAllPastRequestsIds = async (mocked) => {
    const eventDecryption = await gateway.filters
        .EventDecryption()
        .getTopicFilter();
    ...
    for (const request of pastRequests) {
        ...
        const typesList = handles.map((handle) =>
            parseInt(handle.toString(16).slice(-4, -2), 16),
        );
            ...
            if (mocked) {
                ...
                const isAllowedForDec = await Promise.all(
                    handles.map(async (handle) =>
                        acl.allowedForDecryption(handle),
                    ),
                );
                if (!allTrue(isAllowedForDec)) {
                    throw new Error(
                        'Some handle is not authorized for decryption',
                    );
                }
                ...
                const values = await Promise.all(
                    handles.map(async (handle) =>
                        BigInt(await getClearText(handle)),
                    ),
                );
                ...
                const valuesFormatted2 = valuesFormatted.map((value, index) =>
                    types[index] === 'bytes'
                        ? '0x' + value.toString(16).padStart(512, '0')
                        : value,
                );
                ...
                const tx = await gateway
                    .connect(relayer)
                    .fulfillRequest(requestID, calldata, [], {
                        value: msgValue,
                    });
                await tx.wait();
            } else {
