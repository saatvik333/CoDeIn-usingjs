...
const coprocAdd = parsedEnvCoprocessor.TFHE_EXECUTOR_CONTRACT_ADDRESS.replace(
    /^0x/,
    '',
)
    .replace(/^0+/, '')
...
export const getClearText = async (handle) => {
            ...
            db.get(
                ...
                (err, row) => {
                    if (err) {
                        ...
                        reject(
                            new Error('No record found after maximum retries'),
                        );
                    }
                },
            );
...
function extractCalldata(memory, offset, size) {
...
export function numberToEvenHexString(num) {
...
function appendType(handle, type) {
    return (
        handle.slice(0, -4) +
        numberToEvenHexString(type) +
        numberToEvenHexString(HANDLE_VERSION)
    );
}
...
async function insertHandle(obj2, validIdxes) {
    ...
    if (isCoprocAdd(obj.stack.at(-2))) {
        ...
        switch (selectors[currentSelector]) {
            ...
            case 'fheAdd(uint256,uint256,bytes1)':
                ...
                lhsType = parseInt(
                    decodedData[0].toString(16).slice(-4, -2),
                    16,
                );
                resultType = lhsType;
                handle = appendType(handle, resultType);
            ...
            case 'fheShl(uint256,uint256,bytes1)':
                ...
                if (decodedData[2] === '0x01') {
                    clearText =
                        BigInt(clearLHS) <<
                        decodedData[1] % NumBits[resultType];
                    clearText = clearText % 2n ** NumBits[resultType];
                } else {
                    clearRHS = await getClearText(decodedData[1]);
                    clearText =
                        BigInt(clearLHS) <<
                        BigInt(clearRHS) % NumBits[resultType];
                    clearText = clearText % 2n ** NumBits[resultType];
                }
                insertSQL(handle, clearText);
                break;
            ...
            case 'fheMax(uint256,uint256,bytes1)':
                ...
                if (decodedData[2] === '0x01') {
                    clearText =
                        BigInt(clearLHS) > decodedData[1]
                            ? clearLHS
                            : decodedData[1];
                } else {
                    clearRHS = await getClearText(decodedData[1]);
                    clearText =
                        BigInt(clearLHS) > BigInt(clearRHS)
                            ? clearLHS
                            : clearRHS;
                }
                insertSQL(handle, clearText);
            ...
            case 'cast(uint256,bytes1)':
                ...
                handle = ethers.utils.keccak256(
                    ethers.utils.solidityPack(
                        ['uint8', 'uint256', 'bytes1'],
                        [Operators.cast, decodedData[0], decodedData[1]],
                    ),
                );
                clearText =
                    BigInt(await getClearText(decodedData[0])) %
                    2n ** NumBits[resultType];
                handle = appendType(handle, resultType);
                insertSQL(handle, clearText);
                break;
            ...
            case 'fheNot(uint256)':
                ...
                handle = ethers.utils.keccak256(
                    ethers.utils.solidityPack(
                        ['uint8', 'uint256'],
                        [Operators.fheNot, decodedData[0]],
                    ),
                );
                handle = appendType(handle, resultType);
                clearText = BigInt(await getClearText(decodedData[0]));
                clearText = bitwiseNotUintBits(
                    clearText,
                    Number(NumBits[resultType]),
                );
                insertSQL(handle, clearText);
                break;
            ...
            case 'fheNeg(uint256)':
                ...
                handle = ethers.utils.keccak256(
                    ethers.utils.solidityPack(
                        ['uint8', 'uint256'],
                        [Operators.fheNeg, decodedData[0]],
                    ),
                );
                handle = appendType(handle, resultType);
                clearText = BigInt(await getClearText(decodedData[0]));
            ...
            case 'fheIfThenElse(uint256,uint256,uint256)':
                {
                    resultType = parseInt(
                        decodedData[1].toString(16).slice(-4, -2),
                        16,
                    );
                    handle = ethers.utils.keccak256(
                        ethers.utils.solidityPack(
                    ...
                    handle = appendType(handle, resultType);
                    const clearControl = BigInt(
                        await getClearText(decodedData[0]),
                    );
                    const clearIfTrue = BigInt(
                        await getClearText(decodedData[1]),
                    );
                    const clearIfFalse = BigInt(
                        await getClearText(decodedData[2]),
                    );
                    if (clearControl === 1n) {
            ...
            case 'fheRand(bytes1)':
                if (validIdxes.includes(obj2.index)) {
                    ...
                    handle = ethers.utils.keccak256(
                        ethers.utils.solidityPack(
                            ['uint8', 'bytes1', 'uint256'],
                            [Operators.fheRand, decodedData[0], counterRand],
                        ),
                    );
                    handle = appendType(handle, resultType);
            ...
            case 'fheRandBounded(uint256,bytes1)':
                if (validIdxes.includes(obj2.index)) {
                    ...
                    handle = appendType(handle, resultType);
                    clearText = getRandomBigInt(
                        Number(log2(BigInt(decodedData[0]))),
                    );
                    insertSQL(handle, clearText, true);
                    counterRand++;
                }
...
function bitwiseNotUintBits(value, numBits) {
    ...
    if (typeof numBits !== 'number' || numBits <= 0) {
        throw new TypeError(
            'The numBits parameter must be a positive integer.',
        );
    }
...
function isCoprocAdd(longString) {
...
export const awaitCoprocessor = async () => {
