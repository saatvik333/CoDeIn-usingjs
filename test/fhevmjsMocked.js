...
function createUintToUint8ArrayFunction(numBits) {
    ...
    return function (uint) {
        ...
        let totalBuffer;
        const padBuffer =
            numBytes <= 20 ? Buffer.alloc(20 - numBytes) : Buffer.alloc(0); // to fit it in an E160List
...
export const reencryptRequestMocked = async (
    ...
    const normalizedSignerAddress = ethers.utils.getAddress(signerAddress);
    ...
    // ACL checking
    const aclFactory = new ethers.ContractFactory(
        (await import('fhevm/lib/ACL.sol:ACL')).default.abi,
        (await import('fhevm/lib/ACL.sol:ACL')).default.bytecode,
...
export const createEncryptedInputMocked = (contractAddress, callerAddress) => {
                ...
                case 160: {
                    bits.map((v, i) => {
                        encrypted = Buffer.concat([
                            encrypted,
                            createUintToUint8ArrayFunction(v)(values[i]),
                        ]);
                    });
                    break;
                }
